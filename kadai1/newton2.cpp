...
int _tmain(int argc, _TCHAR *argv[])
{
	...
	else
	{
		...
		while (1)
		{
			...
			g[2] = A[1][1] * x[1] + 2 * A[0][2] * x[2] + A[0][1];
			...
			// ヘッセ行列
			h[1][1] = 12 * A[4][0] * pow(x[1], 2) + 6 * A[3][0] * x[1] + 2 * A[2][0];
			h[1][2] = A[1][1];
			h[2][1] = A[1][1];
			h[2][2] = 2 * A[0][2];
...
double comp_val(double *x)
{
	...
	val = A[4][0] * pow(x[1], 4) + A[3][0] * pow(x[1], 3) + A[2][0] * pow(x[1], 2) + A[1][0] * x[1] + A[1][1] * x[1] * x[2] + A[0][2] * pow(x[2], 2) + A[0][1] * x[2];
	return val;
}
